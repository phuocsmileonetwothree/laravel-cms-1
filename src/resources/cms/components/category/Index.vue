                ...
                <table class="w-full text-left border-b border-slate-200/60">
                    ...
                    <thead v-if="data" ref="dataTable">
                        <tr v-for="item in data" :key="item">
                            ...
                            <td class="px-5 border-b dark:border-darkmode-300 py-4 border-dashed dark:bg-darkmode-600">
                                <router-link :to="{ name: route.nameComponentEdit, params: { id: item.id } }" class="font-medium whitespace-nowrap">{{ item.title }}</router-link>
...
export default {
    ...
    methods: {
        ...
        load() {
            axios
                .get(`${this.baseURL}/${this.route.nameAPI}?paginate=true`, {
                    params: this.filters,
                })
                .then((response) => {
